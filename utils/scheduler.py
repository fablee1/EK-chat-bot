...
async def start_apscheduler():
    scheduler.start()
    scheduler.add_job(remind_rep, trigger='cron', second=48, id="remind_rep", replace_existing=True, misfire_grace_time=60*60*12)
    scheduler.add_job(remind_prize, trigger='cron', second=12, id="remind_prize", replace_existing=True, misfire_grace_time=60*60*12)
